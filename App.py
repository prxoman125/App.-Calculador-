...
st.markdown("""
    ...
    .titulo-container {
        background: linear-gradient(135deg, #020617, #0f172a, #1e293b, #0f172a, #020617);
        background-size: 200% 200%;
        animation: fondoSuave 16s ease-in-out infinite;
        padding: 24px;
        border-radius: 12px;
        text-align: center;
        margin-bottom: 28px;
    ...
    @keyframes fondoSuave {
        0%, 100% { background-position: 0% 50%; border-color: #1e293b; }
        50% { background-position: 100% 50%; border-color: #334155; }
    }
...
def agregar_a_historial(sector, detalle, resultado):
...
TEXTOS = {
    "Español": {
        ...
        "tech_sub": "💻 Sector Tecnológico", "tech_u": "Número de usuarios activos:", "tech_c": "Costo mensual por usuario ($):", "tech_d": "Descuento aplicado (%):", "tech_btn": "Calcular Total", "tech_res": "Costo Total Mensual: ${:.2f}",
        "man_sub": "⚙️ Sector Manufactura", "man_u": "Unidades a producir:", "man_m": "Costo de material por unidad ($):", "man_f": "Costos operativos fijos ($):", "man_btn": "Calcular Costo de Producción", "man_res1": "Costo de Producción Total: ${:.2f}", "man_res2": "Costo por unidad fabricada: ${:.2f}",
        "com_sub": "🛍️ Sector Comercio", "com_c": "Costo de adquisición del producto ($):", "com_m": "Margen de ganancia deseado (%):", "com_i": "Impuesto local / IVA (%):", "com_btn": "Calcular Precio de Venta", "com_res1": "Precio de Venta al Público: ${:.2f}", "com_res2": "Ganancia neta por producto: ${:.2f}",
        "sal_sub": "🏥 Sector Salud", "sal_c": "Número de consultas estimadas:", "sal_p": "Precio base por consulta ($):", "sal_o": "Costos operativos por consulta ($):", "sal_btn": "Calcular Rentabilidad Clínica", "sal_res1": "Ingresos Brutos Estimados: ${:.2f}", "sal_res2": "Ganancia Neta Estimada: ${:.2f}",
        "con_sub": "🏗️ Sector Construcción", "con_m": "Metros cuadrados (m²):", "con_c": "Costo de materiales por m² ($):", "con_o": "Costo de mano de obra por m² ($):", "con_btn": "Calcular Presupuesto Obra", "con_res1": "Presupuesto Total Estimado: ${:.2f}", "con_res2": "Costo Total por m²: ${:.2f}",
        "hist_titulo": "📊 Historial de Cálculos (Máx. 5)", "hist_vacio": "Aún no hay registros. Haz un cálculo para guardar.", "btn_borrar": "🗑️ Borrar tablas / historial", "borrado_ok": "Historial borrado correctamente."
    },
    "English": {
        ...
        "hist_titulo": "📊 Calculation History (Max. 5)", "hist_vacio": "No records yet. Make a calculation to save.", "btn_borrar": "🗑️ Clear tables / history", "borrado_ok": "History cleared successfully."
    }
}
...
if sector in ["Tecnología / Software", "Technology / Software"]:
    ...
    if st.button(txt["tech_btn"]):
        ...
        agregar_a_historial(sector, f"{usuarios} users x ${costo_por_usuario} -{descuento}%", f"${total:.2f}")
    ...
    if st.button(txt["man_btn"]):
        ...
        agregar_a_historial(sector, f"{unidades}u x ${costo_material} + ${costo_operativo_fijo}", f"${total:.2f}")
    ...
    if st.button(txt["com_btn"]):
        ...
        agregar_a_historial(sector, f"Costo ${costo_producto} + {margen_ganancia}% + {impuesto}% IVA", f"${precio_final:.2f}")
    ...
    if st.button(txt["sal_btn"]):
        ...
        agregar_a_historial(sector, f"{consultas} consultas x ${precio_consulta}", f"${ganancia:.2f} neta")
    ...
    if st.button(txt["con_btn"]):
        ...
        agregar_a_historial(sector, f"{metros} m² x (${costo_material_m2}+${costo_obra_m2})", f"${total:.2f}")
